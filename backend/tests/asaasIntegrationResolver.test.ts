...
test('resolveAsaasIntegration prioriza credenciais da empresa quando escopo empresarial é solicitado', async () => {
  const companyRow = {
    id: 21,
    provider: 'asaas',
    url_api: null,
    key_value: 'company-token',
  ...
  const pool = {
    ...
    async query(text: string, params?: unknown[]) {
      ...
      const preferCompany = /CASE WHEN i.idempresa = \$2 THEN 0 ELSE 1 END/.test(text);
      const rows = preferCompany ? [companyRow, globalRow] : [globalRow, companyRow];
      return { rows, rowCount: rows.length };
    },
  };

  const integration = await resolveAsaasIntegration(EMPRESA_ID, pool as any, undefined, {
    scope: 'company-first',
  });

  assert.equal(integration.integrationId, 21);
  assert.equal(integration.accessToken, 'company-token');
  assert.match(pool.calls[0]?.text ?? '', /CASE WHEN i.idempresa = \$2 THEN 0 ELSE 1 END/);
});

test('resolveAsaasIntegration prioriza credenciais globais quando solicitado', async () => {
  const companyRow = {
    id: 31,
    provider: 'asaas',
    url_api: null,
  ...
  const pool = {
    ...
    async query(text: string, params?: unknown[]) {
      calls.push({ text, params });
      const preferGlobal = /CASE WHEN i.global IS TRUE THEN 0 ELSE 1 END/.test(text);
      const rows = preferGlobal ? [globalRow, companyRow] : [companyRow, globalRow];
      return { rows, rowCount: rows.length };
    },
  };
  ...
  assert.equal(integration.integrationId, 32);
  assert.equal(integration.accessToken, 'global-token');
  assert.match(pool.calls[0]?.text ?? '', /CASE WHEN i.global IS TRUE THEN 0 ELSE 1 END/);
});
