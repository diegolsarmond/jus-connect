...
export const listClienteDocumentos = async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(
      `SELECT cd.id, cd.nome_arquivo, cd.tipo_documento_id, td.nome AS tipo_nome,
...
export const createClienteDocumento = async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO public.cliente_documento (cliente_id, tipo_documento_id, nome_arquivo, arquivo_base64)
       VALUES ($1, $2, $3, $4)
       RETURNING id, nome_arquivo, tipo_documento_id, data_upload`,
      [clienteId, tipo_documento_id, nome_arquivo, arquivo_base64]
    );
    const row = result.rows[0];
    const tipo = await pool.query(
      'SELECT nome FROM public.tipo_documento WHERE id = $1',
...
export const deleteClienteDocumento = async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(
      'DELETE FROM public.cliente_documento WHERE id = $1',
      [id]
    );
