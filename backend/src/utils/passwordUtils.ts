...
const loadArgon2 = async (): Promise<Argon2Module> => {
    // If caller requested forcing the fallback, ensure we use and cache it.
    if (shouldForceFallback()) {
        if (argon2ModuleSource !== 'fallback' || !argon2ModulePromise) {
    ...
    // If we already have any module promise (native or fallback), reuse it.
    if (argon2ModulePromise) {
        return argon2ModulePromise;
    }

    // First attempt to load native argon2. If it fails, replace the cached promise
    // with the fallback module promise so subsequent calls reuse the fallback.
    argon2ModulePromise = import('argon2')
        ...
        .catch(async () => {
            // mark fallback and cache the fallback promise to avoid repeated failed imports
            argon2ModuleSource = 'fallback';
            const fallback = await loadFallbackModule();
            // replace the current promise with an already-resolved fallback promise
            argon2ModulePromise = Promise.resolve(fallback);
            return fallback;
        });
