-- public.vw_processos_sync_targets fonte

CREATE OR REPLACE VIEW public.vw_processos_sync_targets
AS WITH period_bounds AS (
         SELECT emp_1.id AS empresa_id,
            COALESCE(emp_1.current_period_start, date_trunc('month'::text, now())) AS period_start,
            COALESCE(emp_1.current_period_end, COALESCE(emp_1.current_period_start, date_trunc('month'::text, now())) + '1 mon'::interval) AS period_end
           FROM empresas emp_1
        )
 SELECT m.id AS monitor_id,
    ...
    COALESCE(sync_stats.processos_sincronizadas_periodo, 0::bigint) AS processos_sincronizadas_periodo,
        CASE
            ...
            ELSE GREATEST(pl.sincronizacao_processos_limite - COALESCE(sync_stats.processos_sincronizadas_periodo, 0::bigint), 0::bigint)
   ...
   FROM processo_oab_monitoradas m
     ...
     LEFT JOIN LATERAL ( SELECT count(*) AS processos_sincronizadas_periodo
           FROM processos p
          WHERE p.idempresa = emp.id AND (period.period_start IS NULL OR p.criado_em >= period.period_start) AND (period.period_end IS NULL OR p.criado_em < period.period_end)) sync_stats ON true;