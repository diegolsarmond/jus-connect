...
exports.__test__ = exports.deleteOpportunityDocument = exports.getOpportunityDocument = exports.listOpportunityDocuments = exports.createOpportunityDocumentFromTemplate = void 0;
...
async function resolveAuthenticatedContext(req, res) {
    if (!req.auth) {
...
async function ensureOpportunityAccess(oportunidadeId, empresaId) {
...
function parseStoredDocumentContent(raw) {
    if (!raw) {
        return { contentHtml: '<p></p>', contentEditorJson: null, metadata: null };
    }
    try {
        const parsed = JSON.parse(raw);
        const contentHtml = typeof parsed?.content_html === 'string' && parsed.content_html.trim().length > 0
            ? parsed.content_html
            : '<p></p>';
        return {
            contentHtml,
...
function collectNodePlainText(node) {
    let text = '';
...
function fillEditorNodes(nodes, variables) {
    return nodes.map((node) => {
        const dataVariable = node.attrs && typeof node.attrs['data-variable'] === 'string'
            ? node.attrs['data-variable']
            : null;
        if (dataVariable) {
            const resolved = (0, templateService_1.resolveVariableValue)(variables, dataVariable);
...
function formatTimeString(input, fallbackDate) {
...
function formatAudienceTime(date, time) {
...
const isPgMissingRelationError = (error) => {
...
async function fetchEmpresaEndereco(empresaId) {
    if (!empresaId) {
        return null;
    }
    for (const source of EMPRESA_ADDRESS_SOURCES) {
        const columnsResult = await db_1.default
            .query(`SELECT column_name
           FROM information_schema.columns
          WHERE table_schema = $1 AND table_name = $2`, [source.schema, source.table])
            .catch((error) => {
...
async function fetchOpportunityProcessId(opportunityId) {
    const queries = [
...
`;
const buildAgendaAudienceQuery = (column) => `
  SELECT a.data, a.hora_inicio AS hora, a.local
   ...
   LIMIT 1
`;
async function fetchOpportunityAudience(opportunity) {
    const attempts = [
    ...
    for (const attempt of attempts) {
        const result = await db_1.default
            .query(attempt.query, attempt.params)
            .catch((error) => {
            if (isPgMissingRelationError(error)) {
                return null;
            }
...
function buildVariables({ opportunity, solicitante, envolvidos, responsavel, empresa, audiencia, }) {
    const variables = {};
    ...
    assign('processo.audiencia.horario', formatTimeString(opportunity.audiencia_horario, opportunity.audiencia_data));
...
async function fetchOpportunityData(id, empresaId) {
    const opportunityResult = await db_1.default.query(`SELECT id, tipo_processo_id, area_atuacao_id, responsavel_id, idempresa, numero_processo_cnj, numero_protocolo,
       ...
       FROM public.oportunidades WHERE id = $1 AND idempresa = $2`, [id, empresaId]);
    ...
    if (opportunity.idempresa !== null && opportunity.idempresa !== undefined) {
        const empresaResult = await (0, empresaQueries_1.queryEmpresas)('WHERE id = $1', [opportunity.idempresa]);
...
const createOpportunityDocumentFromTemplate = async (req, res) => {
    ...
    try {
        ...
        const templateResult = await db_1.default.query('SELECT id, title, content FROM templates WHERE id = $1 AND idempresa IS NOT DISTINCT FROM $2 AND idusuario = $3', [numericTemplateId, empresaId, userId]);
       ...
       RETURNING id, oportunidade_id, template_id, title, content, variables, created_at`, [
        ...
        const responseTitle = document && typeof document.title === 'string' && document.title.trim().length > 0
            ? document.title.trim()
            : finalTitle;
...
const listOpportunityDocuments = async (req, res) => {
    const { id } = req.params;
    const opportunityId = Number(id);
    if (!Number.isFinite(opportunityId)) {
        return res.status(400).json({ error: 'Oportunidade inválida' });
    ...
    try {
        ...
        const result = await db_1.default.query(`SELECT id, oportunidade_id, template_id, title, content, variables, created_at
       FROM public.oportunidade_documentos
       WHERE oportunidade_id = $1
       ORDER BY created_at DESC, id DESC`, [opportunityId]);
...
exports.listOpportunityDocuments = listOpportunityDocuments;
const getOpportunityDocument = async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.default.query(`SELECT id, oportunidade_id, template_id, title, content, variables, created_at
       FROM public.oportunidade_documentos
       WHERE oportunidade_id = $1 AND id = $2`, [opportunityId, docId]);
...
exports.getOpportunityDocument = getOpportunityDocument;
const deleteOpportunityDocument = async (req, res) => {
    const { id, documentId } = req.params;
    const opportunityId = Number(id);
    const docId = Number(documentId);
    if (!Number.isFinite(opportunityId) || !Number.isFinite(docId)) {
    ...
    try {
        ...
        const result = await db_1.default.query('DELETE FROM public.oportunidade_documentos WHERE oportunidade_id = $1 AND id = $2 RETURNING id', [opportunityId, docId]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: 'Documento não encontrado' });
        }
...
exports.deleteOpportunityDocument = deleteOpportunityDocument;
exports.__test__ = {
    buildVariables,
