...
export const listTarefas = async (_req, res) => {
    try {
        const result = await pool.query(`SELECT t.id, t.id_oportunidades, t.titulo, t.descricao, t.data, t.hora,
...
export const getResponsavelByTarefa = async (req, res) => {
    const { id } = req.params;
    try {
...
export const createTarefa = async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(`INSERT INTO public.tarefas (id_oportunidades, titulo, descricao, data, hora, dia_inteiro, prioridade, mostrar_na_agenda, privada, recorrente, repetir_quantas_vezes, repetir_cada_unidade, repetir_intervalo, criado_em, atualizado_em, concluido)
...
export const updateTarefa = async (req, res) => {
    ...
    const unidadeFormatada = repetir_cada_unidade
        ...
        : null;
    try {
        const result = await pool.query(`UPDATE public.tarefas SET id_oportunidades = $1, titulo = $2, descricao = $3, data = $4, hora = $5, dia_inteiro = $6, prioridade = $7, mostrar_na_agenda = $8, privada = $9, recorrente = $10, repetir_quantas_vezes = $11, repetir_cada_unidade = $12, repetir_intervalo = $13, concluido = $14, atualizado_em = NOW() WHERE id = $15
       RETURNING id, id_oportunidades, titulo, descricao, data, hora, dia_inteiro, prioridade, mostrar_na_agenda, privada, recorrente, repetir_quantas_vezes, repetir_cada_unidade, repetir_intervalo, criado_em, atualizado_em, concluido`, [
            id_oportunidades,
            titulo,
            descricao,
...
export const concluirTarefa = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query(`UPDATE public.tarefas SET concluido = true, atualizado_em = NOW() WHERE id = $1 RETURNING id, id_oportunidades, titulo, descricao, data, hora, dia_inteiro, prioridade, mostrar_na_agenda, privada, recorrente, repetir_quantas_vezes, repetir_cada_unidade, repetir_intervalo, criado_em, atualizado_em, concluido`, [id]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: 'Tarefa não encontrada' });
        }
        res.json(result.rows[0]);
