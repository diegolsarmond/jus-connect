...
function resolveVariableValue(values, key) {
...
function replaceVariables(content, values) {
    const withSpansReplaced = content.replace(DATA_VARIABLE_SPAN_REGEX, (_match, _quote, key, innerHtml) => {
    ...
    return withSpansReplaced.replace(MUSTACHE_VARIABLE_REGEX, (_match, key) => {
        const resolved = resolveVariableValue(values, key);
