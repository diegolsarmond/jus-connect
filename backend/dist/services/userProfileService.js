...
const normalizeOptionalString = (value, { uppercase = false } = {}) => {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
...
const mapSpecialtiesFromRow = (value) => {
    if (!Array.isArray(value)) {
        return [];
    }
    const result = [];
...
const buildProfileStorage = (row) => ({
    ...
    notifications_security_alerts: row?.notifications_security_alerts ?? true,
    notifications_agenda_reminders: row?.notifications_agenda_reminders ?? true,
...
const mapAuditLogRow = (row) => ({
    ...
    performedBy: row.performed_by === null || row.performed_by === undefined
        ? null
        : Number(row.performed_by),
    performedByName: typeof row.performed_by_name === 'string' ? row.performed_by_name : null,
...
class UserProfileService {
    ...
    async fetchUserProfileRow(userId) {
    ...
    async updateProfile(userId, input, performedBy) {
        const normalizedId = normalizeUserId(userId);
        ...
        const updatesUsuarios = [];
        const valuesUsuarios = [];
        const addUsuarioUpdate = (column, value) => {
        ...
        if (input.notifications) {
            ...
            const securityAlerts = normalizeOptionalBoolean(notifications.securityAlerts);
            if (securityAlerts !== undefined) {
                storage.notifications_security_alerts = securityAlerts;
            }
            const agendaReminders = normalizeOptionalBoolean(notifications.agendaReminders);
        ...
        if (input.security) {
            ...
            const deviceApproval = normalizeOptionalBoolean(security.deviceApproval);
        ...
        if (updatesUsuarios.length > 0) {
            ...
            await this.db.query(`UPDATE public.usuarios SET ${setClause} WHERE id = $${valuesUsuarios.length}`, valuesUsuarios);
        }
        const profileValues = [
            normalizedId,
            storage.title,
        ...
        ON CONFLICT (user_id) DO UPDATE SET
          ...
          updated_at = NOW()
      `, profileValues);
        const changedFieldLabels = new Set();
        const auditEntries = [];
        ...
        if (input.notifications) {
            ...
            if (notifications.agendaReminders !== undefined &&
        ...
        if (input.security) {
            ...
            if (security.deviceApproval !== undefined &&
        ...
        const performerId = performedBy?.id && Number.isInteger(performedBy.id)
            ? Number(performedBy.id)
            : normalizedId;
        const performerName = typeof performedBy?.name === 'string' && performedBy.name.trim()
    ...
    async createAuditLog(userId, action, description, performer) {
        const normalizedId = normalizeUserId(userId);
        ...
        const performerId = performer?.id !== undefined && performer.id !== null
            ? Number(performer.id)
            : null;
        const performerName = typeof performer?.name === 'string' && performer.name.trim()
            ? performer.name.trim()
            : null;
        await this.db.query(`
    ...
    async revokeSession(userId, sessionId, performer) {
        const normalizedId = normalizeUserId(userId);
      ...
      `, [normalizedId, sessionId]);
        ...
        await this.createAuditLog(normalizedId, 'STATUS_CHANGE', `Sessão revogada: ${session.device}.`, performer);
        return session;
    }
    async revokeAllSessions(userId, performer) {
        const normalizedId = normalizeUserId(userId);
        const result = await this.db.query(`
        UPDATE public.user_profile_sessions
        SET is_active = FALSE,
      ...
      `, [normalizedId]);
        ...
        if (revokedCount > 0) {
            await this.createAuditLog(normalizedId, 'STATUS_CHANGE', 'Todas as sessões ativas foram revogadas.', performer);
        }
        return revokedCount;
    }
}
