...
const normalizeProcessSyncSubtotal = (value: unknown): ProcessSyncSubtotal | null => {
  ...
  const service =
    normalizeString(record.service) ??
    normalizeString(record.name) ??
    normalizeString(record.description) ??
    normalizeString(record.code) ??
    null;
  const quantityCandidate =
    normalizeNumber(record.quantity) ?? normalizeNumber(record.count) ?? normalizeNumber(record.requests);
  const totalCandidate =
    normalizeNumber(record.totalCost) ?? normalizeNumber(record.total_cost) ?? normalizeNumber(record.total) ??
  ...
  return {
    service,
    quantity: quantityCandidate != null ? Math.max(0, Math.round(quantityCandidate)) : 0,
    totalCost: totalCandidate != null ? Number(totalCandidate.toFixed(2)) : 0,
  };
};

const normalizeProcessSyncReport = (payload: unknown): ProcessSyncReport => {
  if (!payload || typeof payload !== "object") {
    throw new Error("Resposta inválida do relatório de sincronização da Judit.");
  }
  const periodPayload = (payload as { period?: unknown }).period;
  const period = (() => {
    if (periodPayload && typeof periodPayload === "object") {
      const from = normalizeString((periodPayload as { from?: unknown }).from);
      const to = normalizeString((periodPayload as { to?: unknown }).to);
      return { from: from ?? null, to: to ?? null };
    }
    return { from: null, to: null };
  })();
  const totalRequestsCandidate = normalizeNumber(
    (payload as { totalRequests?: unknown }).totalRequests ??
      (payload as { total_requests?: unknown }).total_requests,
  );
  const totalCostCandidate = normalizeNumber(
    (payload as { totalCost?: unknown }).totalCost ??
      (payload as { total_cost?: unknown }).total_cost ??
      (payload as { total?: unknown }).total,
  );
  const subtotalsRaw = (() => {
    const direct = (payload as { subtotals?: unknown }).subtotals;
    if (Array.isArray(direct)) {
      return direct;
    }
    if (direct && typeof direct === "object" && Array.isArray((direct as { data?: unknown }).data)) {
      return (direct as { data: unknown[] }).data;
    }
    return [];
  })();
  const subtotals = subtotalsRaw
    .map((item) => normalizeProcessSyncSubtotal(item))
    .filter((item): item is ProcessSyncSubtotal => item !== null);
  return {
    period,
    totalRequests: totalRequestsCandidate != null ? Math.max(0, Math.round(totalRequestsCandidate)) : 0,
    totalCost: totalCostCandidate != null ? Number(totalCostCandidate.toFixed(2)) : 0,
    subtotals,
  };
};

export async function loadProcessSyncReport(signal?: AbortSignal): Promise<ProcessSyncReport> {
  const response = await fetch(getApiUrl('admin/judit-sync-report'), {
    headers: JSON_HEADERS,
    signal,
  });
  if (!response.ok) {
    throw new Error(`Falha ao carregar relatório de sincronização da Judit (HTTP ${response.status}).`);
  }
  try {
    const payload = await response.json();
    return normalizeProcessSyncReport(payload);
  } catch (error) {
    throw new Error('Resposta inválida do relatório de sincronização da Judit.');
  }
}
