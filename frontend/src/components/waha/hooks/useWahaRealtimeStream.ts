...
export interface WahaRealtimeHandlers {
...
export interface UseWahaRealtimeStreamResult {
  isConnected: boolean;
}
...
const parseSseEvent = (rawEvent: string): { event?: string; data?: unknown } | null => {
  ...
  if (dataString.length > 0) {
    try {
      ...
      console.warn("Failed to parse WAHA SSE payload", error, { dataString });
    }
  }
...
export const useWahaRealtimeStream = (
  handlers: WahaRealtimeHandlers,
): UseWahaRealtimeStreamResult => {
  const { token } = useAuth();
  const [isConnected, setIsConnected] = useState(false);
  const handlersRef = useRef<WahaRealtimeHandlers>(handlers);
      ...
      while (!isCancelled) {
        ...
        try {
          ...
          if (response.status === 401) {
            console.error("WAHA realtime stream unauthorized. Verify authentication token.");
            break;
          }

          if (!response.ok || !response.body) {
            throw new Error(`Unexpected WAHA realtime response: ${response.status}`);
          }
          ...
          console.warn("WAHA realtime stream disconnected", error);
        } finally {
          if (!isCancelled) {
            setIsConnected(false);
      ...
      switch (event) {
        case "conversation:update":
          if (data && typeof data === "object") {
            handlersRef.current.onConversationUpdate?.(data as ConversationSummary);
          }
          break;
        case "message:new":
          if (data && typeof data === "object") {
            handlersRef.current.onMessageNew?.(data as MessageNewPayload);
          }
          break;
        case "message:status":
          if (data && typeof data === "object") {
            handlersRef.current.onMessageStatus?.(data as MessageStatusPayload);
          }
          break;
